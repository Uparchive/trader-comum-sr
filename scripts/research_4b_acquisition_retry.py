...
TARGET=100000
PAGE=1000
NS=(50,150,300,500,600,900,1000)
SEED=20260831
PERMS=10000
PACING_SECONDS=0.30
MAX_PAGES=120
OUT_GZ='data/acquisition/day-04b-retry-2026-09-01.json.gz'
OUT_SUM='data/acquisition/day-04b-retry-2026-09-01-summary.json'
CHECKPOINT='data/acquisition/day-04b-retry-2026-09-01-checkpoint.json'
...
def get(path,attempts=6):
    ...
    for i in range(attempts):
        try:
            req=urllib.request.Request(GATEWAY+path,headers={'User-Agent':'KellQuantLab-Research/4B'})
...
active=get('/research/active-symbols')
rows=active.get('active_symbols') or active.get('data',{}).get('active_symbols') or []
selected=[]
for x in rows:
    ...
    name=' '.join(str(x.get(k,'')) for k in ('display_name','underlying_symbol_name','market_display_name'))
    m=re.search(r'\b(CRASH|BOOM)\s*(50|150|300|500|600|900|1000)\b',(name+' '+str(sym)).upper())
    if sym and m: selected.append((m.group(1),int(m.group(2)),sym,name))
uniq={(a,n):(s,name) for a,n,s,name in selected}
expected=[(a,n) for a in ('BOOM','CRASH') for n in NS]
missing=[k for k in expected if k not in uniq]
if missing: raise RuntimeError('Universo congelado incompleto: '+repr(missing))
universe=[{'family':a,'N':n,'symbol':uniq[(a,n)][0],'name':uniq[(a,n)][1]} for a,n in expected]

checkpoint={'schema_version':1,'cycle':'Dia 4B','cutoff_epoch':cutoff,'target_per_symbol':TARGET,'page_size':PAGE,'symbols':{}}
...
for u in universe:
    ...
    if shortfall:
        rec['status']='ACQUISITION_SHORTFALL'
        results.append(rec); print(u['symbol'],len(items),'SHORTFALL',stop_reason,flush=True); continue
    if len(items)<100:
        rec['status']='INSUFFICIENT_ACQUISITION'; results.append(rec); continue
